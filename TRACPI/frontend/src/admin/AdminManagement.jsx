...
const AdminManagement = () => {
    ...
    return (
        <div className="min-h-screen bg-white w-full">
            ...
            {!showAddAdmin && !showEditAdmin && !showDeleteAdmin && !showSuspendAdmin && !showExportPopup && (
                ...
                >
                        ...
                        <div className="flex gap-2">
                            ...
                            <div className="relative" ref={filterRef}>
                                <button
                                    ...
                                    className={`bg-white border text-gray-700 px-6 py-2 rounded-lg shadow-sm hover:bg-gray-50 transition flex items-center gap-2 ${selectedFilter !== 'All' ? 'border-[#FFB300] bg-[#FFF8E7]' : ''}`}
                                >
                                ...
                                {isFilterOpen && (
                                    <div className="absolute right-0 mt-2 w-48 bg-white border border-[#FFB300] rounded-xl shadow-xl z-[100] overflow-hidden">
                                        {['All', 'Suspended', 'Super Admin', 'Editor', 'Admin'].map((filter) => (
                                            <button
                                                key={filter}
                                                onClick={() => {
                                                ...
                                                className={`w-full text-left px-4 py-3 text-sm hover:bg-[#FFF1CF] transition-colors border-b border-gray-100 last:border-0 ${selectedFilter === filter ? 'bg-[#FFF1CF] font-bold text-[#FF8200]' : 'text-gray-700'}`}
                                            >
                                                {filter}
                                            </button>
                                        ))}
                    ...
                    {!loading && (
                        <div className="overflow-x-auto px-6">
                            <table className="w-full text-left" style={{ borderSpacing: '0 15px', borderCollapse: 'separate' }}>
                                <thead>
                                    <tr className="bg-[#FFB300] text-white h-[60px]">
                                        <th className="px-4 py-2 rounded-l-[10px] font-semibold pl-6">User Name</th>
                                        ...
                                        <th className="px-4 py-2 rounded-r-[10px] text-center font-semibold pr-6">Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                            ...
                                            return (
                                                ...
                                                >
                                                    <td className="px-4 py-2 rounded-l-[10px] border-l border-t border-b border-[#FFB300] pl-6">
                                                        <div className="flex items-center gap-3">
                                                            <input type="radio" name="adminSelect" className="accent-black w-4 h-4 cursor-pointer" />
                                                            <span className="font-medium text-gray-800">{admin.username}</span>
                                                        </div>
                                                    </td>
                                                    <td className="px-4 py-2 border-t border-b border-[#FFB300] text-gray-800">{admin.fullname}</td>
                                                    <td className="px-4 py-2 border-t border-b border-[#FFB300] text-gray-600">{admin.email}</td>
                                                    <td className="px-4 py-2 border-t border-b border-[#FFB300]">
                                                        {admin.status === 'suspended' ? (
                                                            <div className="flex flex-col items-center">
                                                                ...
                                                                <span className="text-[9px] text-gray-500 font-medium">({admin.adminType})</span>
                                                            </div>
                                                        ) : (
                    ...
                    {!loading && (
                        <div className="px-6 mt-4 text-gray-600 font-medium text-sm">
                            No of Users - {currentAdmins.length} out of {filteredAdmins.length}
                        </div>
                    )}
                </div>
