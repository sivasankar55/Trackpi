...
const CourseManagement = () => {
    ...
    const fetchCourses = async () => {
        try {
            ...
            const [coursesRes, statsRes] = await Promise.all([
                ...
                axios.get('http://localhost:5000/api/courses/stats', { withCredentials: true })
            ]);
    ...
    return (
        <div className="min-h-full bg-white w-full font-['Poppins'] pb-12 relative">
            ...
            <div className="sticky top-0 z-[100] flex justify-between items-center px-6 sm:px-10 py-5 bg-white border-b border-gray-100 shadow-sm w-full">
                <div className="flex items-center gap-4 sm:gap-10 flex-1">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-xl sm:text-[24px] font-bold text-[#333] hidden sm:block tracking-tighter">Dashboard</span>
                    </div>
                    ...
                    <div className="relative w-full max-w-[320px]">
                        <input
                            type="text"
                            placeholder="Search"
                            className="w-full h-[40px] pl-6 pr-10 rounded-full border border-gray-200 bg-white placeholder-gray-400 text-sm focus:outline-none focus:ring-1 focus:ring-[#FFB300] transition-all"
                        />
                        <img src={SearchIcon} alt="Search" className="absolute right-5 top-1/2 -translate-y-1/2 w-4 h-4 opacity-30" />
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3 sm:gap-6 ml-4">
                    <button
                        ...
                        className="bg-[#FFA000] hover:bg-[#FF8F00] text-white font-bold px-4 sm:px-8 py-2.5 rounded-[12px] shadow-sm transition-all text-xs sm:text-[14px] whitespace-nowrap active:scale-95"
                    >
                        Add Course
                    </button>
                    ...
                    <div
                        ...
                        className="w-10 h-10 rounded-full border border-gray-200 flex items-center justify-center bg-white shadow-sm cursor-pointer hover:border-[#FFB300] transition-colors overflow-hidden"
                    >
            ...
            <div className="px-6 sm:px-10 mt-8 space-y-10 max-w-[1600px] mx-auto">
                {/* Stats Row */}
                ...
                <div className="w-full">
                    <div className="w-full h-[54px] bg-[#FF9D00] rounded-[15px] flex items-center px-6 shadow-md transition-all hover:shadow-lg focus-within:ring-2 focus-within:ring-orange-200">
                        <img src={SearchIcon} alt="Search" className="w-4 h-4 brightness-0 invert opacity-100" />
                        <input
                            type="text"
                            placeholder="search for student name, course..."
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="bg-transparent border-none outline-none text-white placeholder-white/90 ml-5 w-full text-base font-semibold tracking-wide"
                        />
                    </div>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col justify-center items-center py-40 w-full text-center">
                        <div className="text-red-500 font-bold text-xl mb-4">Error: {error}</div>
                        <button
                    ...
                    <div className="flex flex-col justify-center items-center py-40 w-full text-center">
                        <div className="text-gray-400 font-bold text-xl mb-4">No courses found</div>
                        <p className="text-gray-500 mb-6">Try adjusting your search or add a new course.</p>
                        <button
                            onClick={handleAddCourse}
                    ...
                    <>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6 sm:gap-10 min-h-[600px]">
                            {paginatedCourses.map((course) => {
                                const unitCount = course.sections?.reduce((acc, section) => acc + (section.units?.length || 0), 0) || 0;
                                const totalMinutes = unitCount * 15;
                                const hours = Math.floor(totalMinutes / 60);
                                const mins = totalMinutes % 60;
                                const durationStr = hours > 0 ? `${hours}h ${mins}m` : `${totalMinutes} MINUTES`;

                                return (
                                    <div key={course._id} className="bg-[#FFF8E7] rounded-[35px] overflow-hidden border border-[#FFD9A0] shadow-[0px_4px_25px_rgba(0,0,0,0.04)] flex flex-col h-[520px] transition-all hover:shadow-[0px_8px_35px_rgba(255,183,0,0.15)] group">
                                        {/* Thumbnail Container */}
                                        <div className="p-5">
                                        ...
                                        <div className="px-7 pt-4 flex flex-col flex-1">
                                            ...
                                            <div className="space-y-2 mb-10">
                                                <div className="text-[14px] text-[#333] font-bold">
                                                    Sections: <span className="ml-1">{course.sections?.length || 0}</span>
                                                </div>
                                                <div className="text-[14px] text-[#333] font-bold">
                        ...
                        <div className="flex flex-col sm:flex-row justify-center items-center gap-10 mt-16 mb-4 w-full">
                            <div className="flex items-center gap-4 bg-gray-50 px-3 py-3 rounded-[20px] shadow-sm">
                                <button
                                    onClick={() => handlePageChange(currentPage - 1)}
                                    disabled={currentPage === 1}
                                    className={`px-8 py-2.5 rounded-[12px] bg-[#FFF8E7] text-gray-400 font-bold text-sm shadow-sm transition-all border border-[#FFD9A0]/50 ${currentPage === 1 ? 'opacity-30 cursor-not-allowed' : 'hover:bg-[#FFF3D0] hover:text-[#FF9D00]'}`}
                                >
                                    Previous
                                </button>

                                <div className="flex items-center gap-2">
                                    {Array.from({ length: totalPages || 5 }, (_, i) => i + 1).map((pageNum) => (
                                        <button
                                            key={pageNum}
                                            onClick={() => handlePageChange(pageNum)}
                                            className={`w-10 h-10 rounded-[12px] flex items-center justify-center text-[15px] font-bold transition-all shadow-sm ${currentPage === pageNum
                                ...
                                <button
                                    ...
                                    className={`px-10 py-2.5 rounded-[12px] bg-[#FF9D00] text-white font-bold text-sm shadow-md transition-all ${currentPage === totalPages ? 'opacity-30 cursor-not-allowed' : 'hover:bg-[#E68E00] active:scale-95'}`}
                                >
                                    Next
                                </button>
                            </div>
