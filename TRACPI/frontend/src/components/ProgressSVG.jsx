...
const ProgressSVG = ({ percentage = 0 }) => {
  ...
  // If percentage is exactly 0 and not animating, show the exact unstarted UI provided by user
  if (safePercentage === 0 && percentage === 0) {
    return (
  ...
  // Dynamic values for non-zero percentages
  const center = { x: 101, y: 100 };
  const radiusProgress = 92.5;
  const radiusTicks = 72.5;
  const radiusCenter = 64.5;
  const radiusKnob = 15;

  const circumferenceProgress = 2 * Math.PI * radiusProgress;
  const circumferenceTicks = 2 * Math.PI * radiusTicks;

  // Calculate knob position - Starting from the BOTTOM (90 degrees) and moving CLOCKWISE
  ...
  return (
    <svg width="201" height="232" viewBox="0 0 201 232" fill="none" xmlns="http://www.w3.org/2000/svg">
      <defs>
        ...
        <linearGradient id="progressGradient" x1="101" y1="0" x2="101" y2="200" gradientUnits="userSpaceOnUse">
          <stop stopColor="#FF9407" />
          <stop offset="1" stopColor="#BC6B00" />
        </linearGradient>
        <mask id="ticksMask">
          <circle
            cx={center.x}
      ...
      <circle cx="101.5" cy="100.5" r={radiusTicks} stroke="#A3A3A3" strokeWidth="10" strokeDasharray="2 2" />
      <circle
        cx={center.x}
        cy={center.y}
        r={radiusTicks}
      ...
      {/* Dynamic Knob */}
      <g filter="url(#filter0_d_4302_3545)">
        <circle cx={knobX} cy={knobY} r={radiusKnob} fill="url(#paint1_linear_4302_3545)" />
        <circle cx={knobX} cy={knobY} r={radiusKnob - 0.5} stroke="#F38D07" />
      </g>

      <circle cx="101" cy="101" r={radiusCenter} fill="url(#paint2_linear_4302_3545)" stroke="url(#paint3_linear_4302_3545)" />
